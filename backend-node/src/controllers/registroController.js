...
export const registroController = {
  ...
  async create(req, res) {
    ...
    if (!parsed.success) {
      return res.status(400).json({ ok: false, error: "payload invalido", details: parsed.error.flatten() });
    }
  ...
  async update(req, res) {
    ...
    if (!updated) {
      return res.status(404).json({ ok: false, error: "registro nao encontrado" });
    }
